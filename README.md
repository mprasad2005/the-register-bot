...
The bot creates `pollbot.db` automatically, enables SQLite WAL mode for concurrent announcement workers, and seeds the groups listed in `config.py` on first startup. Set `DB_PATH` to an absolute or deployment-persistent path when using a container or hosting platform with an ephemeral filesystem.
...
## Groups
The three initial groups in `config.py` are placeholders. Replace their numeric IDs with the real Telegram chat IDs before deployment.
...
The main menu includes active giveaways. Authorized admins can create giveaways, publish them to tracked groups, and publish winner results; giveaway records are stored in SQLite.

Announcements accept text and Telegram media/forwarded messages. Each broadcast is stored as a job with one persistent delivery record per recipient and processed by ten asynchronous workers. Telegram rate-limit responses are delayed and retried, blocked recipients are tracked, and unfinished jobs resume after a restart.
